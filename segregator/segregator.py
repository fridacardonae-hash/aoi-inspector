...
class Segregator(ctk.CTk):
    def __init__(self):
        ...
        self.root.bind("<Left>", lambda event: self.prevpic())
        self.root.bind("<Right>", lambda event: self.nextpic())
    ...
    #MANUAL TAB
        ...
        self.button_ok = ctk.CTkButton(tab_manual, text="set OK", width=90, height=80, fg_color="green", command= self.save_ok)
        self.button_ok.grid(row=1, column=4, pady=10, padx=10)

        self.button_ng = ctk.CTkButton(tab_manual, text="set NG", width=90, height=80, fg_color="red", command= self.save_ng)
        self.button_ng.grid(row=1, column=5, pady=10, padx=10, )
    ...
    def showImage(self):
        ...
        self.path_foto = img_path
        output_image,roi, r_mean, g_mean, b_mean, self.ISNs, score, result = self.analyzePic(img_path)
        self.save_log(r_mean,g_mean,b_mean, self.ISNs, score, result)
        ...
        self.isn_label.configure(text=self.ISNs)
        if result == "NG":
    ...
    def nextpic(self, event=None):
        global current_index
        if image_paths and current_index < len(image_paths) - 1:
            current_index += 1
            self.showImage()

    def prevpic(self, event=None):
        global current_index
    ...
    def save_ok (self):
        print("Unidad determinada como OK")
        ok_dir = os.path.join(os.path.dirname(self.path_foto), "OK")
        os.makedirs(ok_dir, exist_ok=True)
        origen = self.path_foto
        destino = ok_dir

        shutil.copy(origen, destino)
        print("Unidad segregada como OK")

    def save_ng (self):
        print("Unidad determinada como NG")
        ng_dir = os.path.join(os.path.dirname(self.path_foto), "NG")
        os.makedirs(ng_dir, exist_ok=True)
        origen = self.path_foto
        destino = ng_dir

        shutil.copy(origen, destino)
        print("Unidad segregada como NG")
